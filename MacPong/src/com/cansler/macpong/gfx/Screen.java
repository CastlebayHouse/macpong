...
public class Screen {
	...
	// 256x256 image. 8x8 pixel tiles, 32 per row, 32 per column ( 32x32 = 1024
	// tiles).
	...
	// Colors is an int that holds four color indexes. These are indexes into
	// the 216 element color palette. Tile range is 0-1023.
	public void render(int xp, int yp, int tile, int colors, int bits) {
		...
		for (int y = 0; y < 8; y++) {
			...
			for (int x = 0; x < 8; x++) {
				...
				if (mirrorX) xs = 7 - x;
				/*
				 * sheet.pixels[xs + ys * sheet.width + toffs] * 8 is simply 0,
				 * 1, 2, or 3 multiplied by 8 to give 0, 8, 16, or 24. The
				 * resulting number is then used to bitshift colors to get a
				 * color palette index
				 * 
				 * pixels[] will be a raster of color indexes. It will have to
				 * be rendered to rgb at some point, using the palette.
				 */
				int col = (colors >> (spriteSheet.pixels[xs + ys * spriteSheet.width + toffs] * 8)) & 255;
	...
	private void blit() {
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				if (x < 256 && y < 256) {
					pixels[x + y * w] = this.spriteSheet.pixels[x + y * spriteSheet.width];
				}
			}
		}
